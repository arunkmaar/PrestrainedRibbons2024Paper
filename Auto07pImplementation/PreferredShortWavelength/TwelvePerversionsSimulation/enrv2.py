...
lvals = list((x * (-0.25) for x in range(200,450)))
...
runModEnergy = runModEnergy + run(runModEnergy('UZ'),ICP =['ea','mExt','l2Tau'], UZSTOP={'ea': -58.9})
...
runModEnergy = run(runModEnergy('UZ'),ICP =['ea','mExt','l2Tau'], ISP=0,UZSTOP={'ea': -57.8},e="enrMZero", DS=1.E-4, UZR={'ea':lvals},NMX=1200)
runModEnergyForward = runModEnergy

runModEnergy = runModEnergy + run(runModEnergy('UZ'),ICP =['ea','mExt','l2Tau'], ISP=2,UZSTOP={'ea': -52.69},e="enrMZero", DS=1.E-4, UZR={'ea':lvals},NMX=1200,STOP={'BP21'},NPR=1)

#runModEnergy = runModEnergy + run(runModEnergy('UZ'),ICP =['ea','mExt','l2Tau'], ISP=0,UZSTOP={'ea': -53.6287},e="enrMZero",NPR=1, NMX=60000,DS = 1.E-10, DSMIN = 1.E-15, DSMAX = 2.E-2,NTST=200,NCOL=4,EPSL = 1e-10, EPSU = 1e-10, EPSS = 1e-7,STOP={'LP1'})

#runModEnergy =  runModEnergy + run(runModEnergy('UZ'),ICP =['ea','mExt','l2Tau'], ISP=2,UZSTOP={'ea': -50.628},e="enrMZero",NPR=1, NMX=100000,DS = 1.E-10, DSMIN = 1.E-15, DSMAX = 2.E-5,NTST=200,NCOL=4,EPSL = 1e-10, EPSU = 1e-10, EPSS = 1e-7)
...
# runModEnergy = relabel(runModEnergy)
runModEnergyForward = run(runModEnergyForward('UZ'), UZSTOP={'ea': -97.053}, DS='-', UZR={'ea':lvals})
# lvals3 = list(((-0.5*x -120) for x in range(1,100)))
# runModEnergyForward = runModEnergyForward + run(runModEnergyForward('UZ'), UZSTOP={'ea': -150.1}, UZR={'ea':lvals3})


runModEnergy += runModEnergyForward
# ######-------------------
lvals2 = list((x * (-5) for x in range(1,15)))
lvals = list(( (0.75-x) for x in range(50,100)))
lvals2.extend(lvals)
runModEnergyInitial= run(enrForward,ICP =['ea','mExt','l2Tau'], ISP=2,UZSTOP={'ea': -54.983},UZR={'ea':lvals2},e="enrMZero",DSMAX=1.E-2,DS=-1.E-4,STOP={'BP4'})
runModEnergy += runModEnergyInitial
